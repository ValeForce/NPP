...
\end{itemize}
\subsubsection{Risonanze}
\begin{itemize}
    \item Esistono risonanze mesoniche. Ci sono $K^*,\rho,\omega$ e $\phi$. Da notare che $\rho$ fa parte di un tripletto mentre le altre hanno isospin nullo, e tutte hanno stranezza nulla (eccetto i $K$).
    \item Inoltre ci sono risonanze barioniche $\Sigma^*$ e $\Xi^*$. 
    \item Dunque è ci sono stati eccitati. Diciamo ciò perché ci ricorda proprio l'atomo che poi decade $\gamma$ o debole. In questo caso però può decadere anche forte.
\end{itemize}
\subsubsection{Scoperta di $\Omega^-$}
\begin{itemize}
\item La particella $\Omega^-$ fu predetta da Gell-Mann sulla base della sua classificazione delle particelle e della interazione forte. 
\item Si cercò questa particella con stranezza pari a tre, trovata tramite una catena di decadimenti.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{immagini/fig_omega_meno.png}
    %\caption{}
    %\label{}
  \end{figure}
\end{itemize}
\subsubsection{Struttura matematica $SU(3)$}
\begin{itemize}
\item È l'insieme di matrici $3\times3$ unitarie a traccia nulla. Sono generate da $3^2-1=8$ matrici indipendenti, detti generatori.
\item Le matrici di Gell-mann sono dunque 8 e le indichiamo con $\lambda_i$ con $i=1,\dots,8$.
\end{itemize}