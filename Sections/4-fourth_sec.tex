...
\begin{itemize}
    ...
    \end{enumerate}
    Dunque conosciamo il rapporto delle sezioni d'urto:\begin{equation*}
    \sigma_1:\sigma_2:\sigma_3 = \abs{M_3}^2:\frac19\abs{M_3+2M_1}^2:\frac29\abs{M_3-M_1}^2
    \end{equation*}
    Nell'ipotesi che la modalità di transizione dominante sia l'una o l'altra, abbiamo \begin{gather*}
...
\begin{gather*}
a+b\to R\to a+b\qquad{\textnormal{reazione elastica}}\\
a+b\to R\to X \qquad{\textnormal{reazione inelastica}}
\end{gather*}
...
\begin{figure}[H]
  ...
  \begin{equation*}
  I_\pm\ket{I,I_3}=\sqrt{I(I+1)-I_3(I_3\pm1)}\ket{I,I_3\pm1}
  \end{equation*}
  ...
  Dunque si definisce
  \begin{equation*}
  \{8,\ket{1,0} \}=\pi^0=\frac1{\sqrt2}\qty(d\bar d-u\bar u)\implies I_+\ket{\pi^0}=\sqrt2\ket{\pi^+}
  \end{equation*}
  ...
  \begin{equation*}
  \eta_8=\{8,\ket{0,0}\}=\frac1{\sqrt6}\qty(u\bar u+d\bar d-2s\bar s)\qquad I_\pm\eta_8=0
  \end{equation*}
...
J=\frac12^+\qquad
\Sigma(1193)\approx\Lambda(1116)\implies
...
\begin{itemize}
    \item Consideriamo la risonanza $\ket{\Delta^{++}}=\ket{\frac32,\frac32}$, con spin dei tre quark $u$ allineati in modo da avere $J=\frac32$ ($l=0$). 
...
\item In realtà, poiché le differenze di massa tra i membri dello stesso multipletti sono $\sim$20\%, la simmetria è "rotta" (cioè approssimata).  
...
\begin{itemize}
    \item Se la $J/\psi$ è uno stato legato di $c\bar c$, allora i mesoni $c\bar q$ e $\bar c q$ devono esistere, con massa circa $m_{J/\psi}+100-200$ MeV ($\frac{3690}2<m_D<\frac{3770}2$).
...
\begin{itemize}
    ...
    \item Ad oggi nessuno lo sa. Il numero di famiglie e le matrici di mescolamento sono parametri liberi del modello standard.
...
\begin{itemize}
    ...
    \item Per $m_t\approx m_W-m_b$ ($\approx 75$ GeV) la ricerca cambiò. Si scoprì il \textit{canale d'oro per la scoperta}, passando dalla ricerca di $W^+\to t\bar b\to W^{+*}b\bar b$ (cioè $W$ virtuale) a $t\to W^+b$. Questo è dovuto al fatto che nessuno pensava che il top fosse così pesante rispetto ai restanti quark.
    \begin{figure}[H]
        \centering
