...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Questi mesoni decadono debolmente. A causa della loro grande massa, le vite medie sono $\Otimes$(ps), cioè un ordine di grandezza più brevi rispetto ai mesoni \(K\).
        \item Il mesone positivo con charm aperto (\(cd̄\), ora chiamato \(D^+\)) decade preferibilmente in stati finali con stranezza negativa (\(c \to sf\bar ƒ\), con \(\Delta S = \Delta C\)). 
    \end{enumerate}
\end{itemize}
\begin{figure}[H]
    \centering
