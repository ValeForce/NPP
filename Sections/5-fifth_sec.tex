...
\begin{itemize}
    ...
    \begin{align*}
        ...
        \end{itemize} 
\end{itemize}
\subsubsection{Modello a partoni}
\begin{itemize}
...
I nucleoni sono fatti da partoni, successivamente identificati con i quark. I partoni sono:
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize} 
\subsubsection{Dimostrazione di $x\_F=x\_B$}
...
\begin{figure}[H]
    ...
    %\caption{Experimental verification of the Callan-Gross relation.}
    %\label{}
\end{figure}
\subsubsection{Riassuntino}
\begin{itemize}
    ...
    \end{align*}
\end{itemize}
\subsubsection{Ricaviamo $\pdv{\sigma}{x}{y}$}
...
\end{itemize}
\subsubsection{Struttura del protone e del neutrone}
...
\begin{equation*}
    ...
    &=x\left\{\frac{4}{9}\qty[u_v(x)+2q_s(x)]+\frac{1}{9}\qty[d_v(x)+2q_s(x)]+\frac{1}{9}\qty[q_s(x)] \right\}=\\
...
\subsubsection{Modello sempificato per $F_2(x)$}
\begin{equation*}
	\begin{cases}
...
\end{equation*}
cioè la differenza, che è un'osservabile, corrisponde approssimaivamente a circa $\frac{1}{3}x$ per la distribuzione in $x$ del singolo quakr di valenza ($u^p_V$ oppure $u^n_V$).
\begin{figure}[H]
    \centering
    ...
    %\label{}
\end{figure}
\subsubsection{I gluoni}
Gli integrali di $F_2(x)$ sono sia calcolabili che misurabili. Trascurando il piccolo contributo di $s\bar s$:
\begin{gather*}
    \int_{0}^{1}F_2^{ep}(x)=\frac{4}{9}\int_{0}^{1}x\qty[u^p(x)+\bar u^p(x)]\dd{x}+\frac{1}{9}\int_{0}^{1}x\qty[d^p(x)+\bar d^p(x)]\dd{x}=\frac{4}{9}f_u+\frac{1}{9}f_d\\
...
\subsubsection{Differenza tra $e^-p$ e $\nu p$ deep inelastic scattering}
Calcoliamo $F_2^{eN}(x)$ per un bersaglio isoscalare\footnote{Serve isoscalare perché, specialmente nello scattering di $\nu$, il bersaglio debe essere pesante, cioè fatto da nuclei pesanti, ben riprodotti dalla approssimazione.}, cioè un bersaglio con $N=Z$, con protoni e neutroni quasi liberi (approssimazione a gas di Fermi):
\begin{align*}
&F_2^{ep}=x\left\{\frac{4}{9}\qty[u^p(x)+\bar u^p(x)]+\frac{1}{9}\qty[d^p(x)+\bar d^p(x)]+\frac{1}{9}\qty[s^p(x)+\bar s^p(x)] \right\}\\
...
\end{itemize}
\subsubsection{Scaling violations}
\begin{minipage}[t]{0.48\textwidth}
    ...
    \begin{figure}[H]
        ...
        %\caption{Violazioni dello scaling di Bjorken.}
        %\label{}
    \end{figure}
\end{minipage}
\hfill
...
\end{figure}
\end{minipage}
Tuttavia questo effetto di scaling violation \textit{non} è dovuto a sottostrutture o nuova fisica, ma ad un cambio dinamico in $F_2$, ben comprenso in QCD. In QCD abbiamo 
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
\subsubsection{Funzione di distribuzione dei partoni}
Per gli esperimenti moderni con adronila conoscenza di $F_2^{p,n}$ è necessario per l'analisi dei dati.
\begin{itemize}
...
\subsubsection{Riassunto sezioni d'urto}
\begin{figure}[H]
    \centering
